import logo from "./logo.svg";
import brightlogo from "./brightlogo.svg";
import quotes from "./quotes.svg";
import send from "./Send.svg";
import shield from "./Shield.svg";
import star from "./Star.svg";
import menu from "./menu.svg";
import close from "./close.svg";
import google from "./google.svg";
import apple from "./apple.svg";
import arrowUp from "./arrow-up.svg";
import discount from "./Discount.svg";
import facebook from "./facebook.svg";
import instagram from "./instagram.svg";
import linkedin from "./linkedin.svg";
import twitter from "./twitter.svg";
import login from "./login.png";
import meta from "./meta.png";
import walet from "./walet.png";
import meta1 from "./meta1.png";
import walet1 from "./walet1.png";
import logoo from "./logoo.png";
import brighterLogo from "./brighterLogo.svg";
import pic1 from "./pic1.jpg";
import EcosystemImage2 from "./ecosystem2.svg";
import Picture from "./picture.png";
import Wallets from "./wallets.png";
import newWallet from "./newWallet.svg";
import CartGold from "./cart-gold.svg";
import CCoinGold from "./c-coin-gold.svg";
import UmbrellaGold from "./umbrella-gold.svg";
import vcard from "./vcard.svg";
import LeafGold from "./leaf-gold.svg";
import SwapGold from "./swap-gold.svg";
import Play from "./play.svg";
import SendDarkIcon from "./send-dark.svg";
import PlusWhiteIcon from "./plus-white.svg";
import XMarkWhiteIcon from "./xmark-white.svg";
import TelegramWhiteIcon from "./telegram-white.svg";
import TelegramGoldIcon from "./telegram-gold.svg";
import TwitterGoldIcon from "./twitter-gold.svg";
import TwitterWhiteIcon from "./twitter-white.svg";
import DiscordGoldIcon from "./discord-gold.svg";
import DiscordWhiteIcon from "./discord-white.svg";
import FacebookGoldIcon from "./facebook-gold.svg";
import FacebookWhiteIcon from "./facebook-white.svg";
import Linkd from "./linkd.svg";
import Linkdin from "./linkedin.svg";
import Linkedin from "./linkdin.svg";
import Dash from "./dash.svg";
import C from "./c.svg";
import Ether from "./ether.svg";
import ETHWhite from "./eth-white.svg";
import DAIWhite from "./eth-white.svg";
import XRPWhite from "./xrp-white.svg";
import XLMWhite from "./xlm-white.svg";
import USDTWhite from "./usdt-white.svg";
import CheckPrimary from "./check-primary.svg";
import Chrysus from "./chrysus.svg";
import Transferblack from "./transfer-black.svg";
import home from "./home.png";
import pic2 from "./pic2.jpg";
import pic3 from "./pic3.jpg";
import pic4 from "./pic4.jpg";
import pic5 from "./pic5.jpg";
import pic6 from "./pic6.jpg";
import G1 from "./G1.png";
import G2 from "./G2.png";
import G3 from "./G3.png";
import G4 from "./G4.png";
import Gov from "./Gov.png";
import G from "./G.png";
import P from "./P.png";
import GI from "./GI.svg";
import A from "./A.svg";
import B from "./B.png";
import CP from "./CP.png";
import D from "./D.png";

import DashboardIcon from "./DashboardIcon.svg";
import GovernanceIcon from "./GovernanceIcon.svg";
import LoanIcon from "./LoanIcon.svg";
import MintIcon from "./MintIcon.svg";
import SwapIcon from "./SwapIcon.svg";
import HomeIcon from "./HomeIcon.svg";
import ero from "./ero.png";
import ero2 from "./ero2.png";
import dai from "./dai.png";
import eths from "./eths.png";
import Loan from "./Loan.png";
import Mint from "./Mint.png";
import Shop from "./Shop.png";
import Swap from "./Swap.png";
import mb from "./mb.svg";
import lb from "./lb.svg";
import sb from "./sb.svg";
import bb from "./bb.svg";
import tick from "./confirm.png";
import market from "./market.png";
import eco from "./eco.png";
import pro1 from "./pro1.png";
import shoe1 from "./shoe1.png";
import zondicons from "./zondicons.svg";
import watch from "./watch.svg";
import sshoe from "./sshoe.svg";
import laptop from "./laptop.svg";
import dshoe from "./dshoe.svg";
import laptopmobile from "./laptopmobile.svg";
import searchshoe from "./searchshoe.svg";
import ecomslide1 from "./ecomslide1.svg";
import leftslidearrow from "./leftslidearrow.svg";
import disclaimer from "./disclaimer.svg";
import rcircle from "./rcircle.svg";
import ycircle from "./ycircle.svg";
import vcardcopy from "./vcardcopy.svg";
import gcircle from "./gcircle.svg";
// import vcardbg from "./vcardbg.svg";
import bgvcard from "./bgvcard.svg";
import vcardchip from "./vcardchip.svg";
import vcardcomingsoon from "./vcardcomingsoon.png";

import vcardleftarrow from "./vcardleftarrow.svg";
import vcardrightarrow from "./vcardrightarrow.svg";
import vcardsignals from "./vcardsignals.svg";
import exclaimatory from "./exclaimatory.svg";
import plusvcard from "./plusvcard.svg";
import filevcard from "./filevcard.svg";
import valid from "./valid.png";

import dao from "./dao-stg.svg";
import bell from "./hal-bell.svg";

import blurmeta from "./blurr-metamask.svg";
import blurwalletconnect from "./blurr-walletconnect.svg";
import blurtrustwallet from "./blurr-trustwallet.svg";
import blureth from "./blurr-ethereum.svg";
import blurlogo from "./blurr-logo.svg";
import goldframe from "./goldframe.svg";
import postblog from "./postblog.svg";
import daogold from "./dao-gold.png";
import groupcoin from "./groupcoin.png";
import groupcoin1 from "./groupcoin1.png";
import heroimgsm from "./hero-img-sm.svg";
import cyberscope from "./cyberscope.png";
import blockapex from "./blockapex.png";
import mapVideo from "./mapVideo.mp4";
import GIFMap from "./GIFMap.gif";
import logos from "./logos.png";
import bg_overlay1 from "./bg_overlay1.jpg";
import lo from "./lo.png";
import op from "./op.svg";
import ex from "./ex.png";
import bl from "./bl.jpg";
import lr from "./lr.png";
import airobo from "./robo.png";
import apivector from "./api-vector.png";

export {
  apivector,
  airobo,
  lr,
  op,
  bl,
  ex,
  lo,
  bg_overlay1,
  logos,
  groupcoin1,
  GIFMap,
  mapVideo,
  blockapex,
  cyberscope,
  heroimgsm,
  groupcoin,
  daogold,
  postblog,
  goldframe,
  blurwalletconnect,
  blurtrustwallet,
  blureth,
  blurlogo,
  blurmeta,
  bell,
  dao,
  shoe1,
  eco,
  market,
  tick,
  mb,
  lb,
  sb,
  bb,
  Swap,
  Shop,
  Mint,
  Loan,
  eths,
  dai,
  ero2,
  ero,
  HomeIcon,
  DashboardIcon,
  GovernanceIcon,
  LoanIcon,
  MintIcon,
  SwapIcon,
  A,
  B,
  CP,
  D,
  GI,
  P,
  G,
  Gov,
  meta1,
  walet1,
  G3,
  G4,
  G2,
  G1,
  pic6,
  pic2,
  pic3,
  pic5,
  pic4,
  home,
  Transferblack,
  Chrysus,
  CheckPrimary,
  USDTWhite,
  XLMWhite,
  XRPWhite,
  DAIWhite,
  ETHWhite,
  Ether,
  C,
  Dash,
  Wallets,
  newWallet,
  Linkedin,
  Linkdin,
  Linkd,
  FacebookWhiteIcon,
  FacebookGoldIcon,
  DiscordWhiteIcon,
  DiscordGoldIcon,
  TwitterWhiteIcon,
  TwitterGoldIcon,
  TelegramGoldIcon,
  TelegramWhiteIcon,
  XMarkWhiteIcon,
  PlusWhiteIcon,
  Picture,
  SendDarkIcon,
  Play,
  SwapGold,
  LeafGold,
  UmbrellaGold,
  CCoinGold,
  CartGold,
  pic1,
  brightlogo,
  logoo,
  logo,
  login,
  quotes,
  send,
  shield,
  star,
  menu,
  close,
  google,
  apple,
  arrowUp,
  discount,
  facebook,
  instagram,
  linkedin,
  twitter,
  walet,
  EcosystemImage2,
  meta,
  zondicons,
  watch,
  sshoe,
  laptop,
  dshoe,
  laptopmobile,
  searchshoe,
  ecomslide1,
  leftslidearrow,
  disclaimer,
  brighterLogo,
  vcard,
  rcircle,
  ycircle,
  gcircle,
  // vcardbg,
  bgvcard,
  vcardchip,
  vcardleftarrow,
  vcardrightarrow,
  vcardsignals,
  exclaimatory,
  plusvcard,
  filevcard,
  vcardcopy,
  valid,
  vcardcomingsoon,
};
